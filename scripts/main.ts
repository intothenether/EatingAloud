...
const languages: Record<'en' | 'pl' | 'sv', string> = {
  en: 'English',
  pl: 'Polski',
  sv: 'Svenska'
};
...
function currentLanguage(): LangKey {
  const stored = localStorage.getItem('siteLang') as LangKey | null;
...
function initLanguageSelector() {
  ...
  select.addEventListener('change', () => setLanguage(select.value as keyof typeof languages));

  desktopMenu.after(select);
}

function updateCardCaptions(lang: keyof typeof captions) {
  document.querySelectorAll('.card-grid .card').forEach((card) => {
    const id = card.getAttribute('data-post-id') as keyof typeof captions.en | null;
...
function applyTranslations() {
  ...
  const ui = translations[lang as keyof typeof translations];
  ...
  if (heroHeader) heroHeader.textContent = ui.heroTitle;
  if (heroSubtitle) heroSubtitle.textContent = ui.heroSubtitle;
  if (ctaBtn) ctaBtn.textContent = ui.ctaButton;

  // more stories
  const moreTitle = document.querySelector('.more-stories h2');
  const moreDesc = document.querySelector('.more-stories > p');
  if (moreTitle) moreTitle.textContent = ui.moreStoriesTitle;
  if (moreDesc) moreDesc.textContent = ui.moreStoriesDesc;
  ...
  if (footerDesc) footerDesc.textContent = ui.footerDesc;
  if (footerCopyright) footerCopyright.textContent = ui.footerCopyright;
  if (footerQuickLinks) footerQuickLinks.textContent = ui.footerQuickLinks;
  if (footerFollowUs) footerFollowUs.textContent = ui.footerFollowUs;
  ...
  if (aboutHeading) aboutHeading.textContent = ui.aboutTitle;
  if (aboutParagraphs[0]) aboutParagraphs[0].textContent = ui.aboutIntro;
  if (aboutParagraphs[1]) aboutParagraphs[1].textContent = ui.aboutMission;
  if (aboutParagraphs[2]) aboutParagraphs[2].textContent = ui.aboutTeam;
  if (aboutParagraphs[3]) aboutParagraphs[3].textContent = ui.aboutContact;
  if (aboutButton) aboutButton.textContent = ui.backToHome;
...
function initSmoothScroll() {
  const down = document.querySelector<HTMLAnchorElement>('.scroll-down');
  if (!down) return;
  down.addEventListener('click', (event) => {
    event.preventDefault();
    const target = document.querySelector<HTMLDivElement>(down.getAttribute('href') || '#');
    if (target) {
